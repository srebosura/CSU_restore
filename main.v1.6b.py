...
class embeddedTerminal(QWidget):
    ...
    def _restore(self):
		...
		self.progress.setRange(0,600)
		self.completed = 0				
		while self.completed < 600:
			time.sleep(0.5)
			self.completed += 1		
			self.progress.setValue(self.completed)
			uboot_flag = os.popen4('cat /tmp/uboot.tmp')[1].read()
			#print(uboot_flag)
			if int(uboot_flag) == 0 and self.completed == 40:
				#print(uboot_flag)
			...
			'tmux', ['send-keys', '-t', 'my_session:0', 'C-z', 'Enter'])
				self._start_process(
			'tmux', ['send-keys', '-t', 'my_session:0', 'Time out (20 secs passed) .... Restart !'])
				self.progress.reset()
				break		
