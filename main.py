...
class embeddedTerminal(QWidget):
    ...
    def _set_housing(self):
		...
		if housing_id == "" or housing_id == "xxxx":
			self._start_process(
			'tmux', ['send-keys', '-t', 'my_session:0', 'HOUSING number missing !'])
			self.housingEdit.setText("xxxx")
			return
		self._start_process(
			'tmux', ['send-keys', '-t', 'my_session:0', 'clear', 'Enter'])
		self._start_process(
    ...
    def _showdialog(self):
		...
		msg8 = QLabel("(*) If calculated gains are Out of Range(+-127), the unit is for repair!")
		msg8.setFont(QFont("Arial",12))
		msg9 = QLabel("(*) You can also enter and trim/adjust the calibration gains (Recommended)....")
		msg9.setFont(QFont("Arial",12))
    ...
    def _batt_calib(self):
		...
		def read_battery():
			...
			comm_check = self.vbat1_r.split(":")[0]
			if str(comm_check) == 'ERROR':
				#print(comm_check)
				self._start_process(
			'tmux', ['send-keys', '-t', 'my_session:0', 'No Comms on CSU serial!'])
				actualV1Edit.setText("Error")
				actualV2Edit.setText("Error")
				vbat1Edit.setText("Error")			
				vbat2Edit.setText("Error")	
				vgain1Edit.setText("Error")	
				vgain2Edit.setText("Error")	
				vbatEdit.setText("Error")
				batselEdit.setText("Error")	
				return
		...
		def calibrate_b1():
			...
			if abs(calib_gain1) >= 127:
				calib_gain1 = "Out of Range"
				vgain1Edit.setText(calib_gain1)
				return
		...
		def calibrate_b2():
			...
			if abs(calib_gain2) >= 127:
				calib_gain2 = "Out of Range"
				vgain2Edit.setText(calib_gain2)
				return
			calib_gain2 = str(calib_gain2)
			vgain2Edit.setText(calib_gain2.split(".")[0])
			
			cmd2 = ['python', 'exec_cmd.py','/dev/'+ self.tty, 'csu_pwrctrl', '-s', 'vbat2_s:'+ calib_gain2.split(".")[0], '10']
			os.popen4(cmd2)
			time.sleep(2)
			self.vbat2_r = os.popen4('python exec_cmd.py /dev/%s "csu_pwrctrl -g vbat2" 10 |tail -2 |head -1'%(self.tty))[1].read()
			b2 = self.vbat2_r.split("=")[1]
